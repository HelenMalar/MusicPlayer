...
const getSongs = async (req, res)=> {
    try {
        const response = await axios.get(`https://api.jamendo.com/v3.0/tracks/?client_id=ea61a820&format=jsonpretty&limit=15`);
...
const getPlaylistByTag = async (req, res)=> {
    try {
        ...
        return res.status(200).json(response.data);
            ...
            return res.status(586).json({ message: "Failed to fetch" });
        }
};
