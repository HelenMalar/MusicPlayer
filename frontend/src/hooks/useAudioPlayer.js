...
function audioReducer (state, action) {
    switch (action.type) {
        ...
        case "TOGGLE_SHUFFLE":
            return {... state, shuffleEnabled: !state.shuffleEnabled, loopEnabledEnabled: false}; 
...
const useAudioPlayer=(songs) =>{
    ...
    const handleTogglePlay = () => {
        ...
        if (audio.paused) {
            ...
            .play()
            .then(()=> dispatch({ type: "PLAY" }))
            .catch((e)=> console.error("Play erroг", e));
        } else {
            audio.pause();
            dispatch({ type: "PAUSE" });
        }
    ...
    const handleEnded = () => {
        ...
        if (audioState.loopEnabled) {
            ...
            .then(()=>{
                dispatch({ type: "PLAY" });
                dispatch({ type: "SET CURRENT_TIME", payload: 0 });
            })
    ...
    return{
        ...
        isLoading: audioState.isLoading,duration,
